#include <float.h>   /* for DBL_MAX (C90-safe) */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
static double *ensure_capacity(double *vals, size_t *cap, size_t needed) {
    size_t new_cap;      /* declare before any statements (C90) */
    double *tmp;
    if (needed <= *cap) return vals;
    new_cap = (*cap == 0) ? 1024 : (*cap * 2);
    while (new_cap < needed) new_cap *= 2;

    tmp = realloc(vals, new_cap * sizeof(double));
    if (!tmp) {
        free(vals);
        handle_error();
    }
...
/* Parse one line of numbers into vals; return number of columns */
static int parse_line(char *line, const char *delims, double **vals, size_t *len, size_t *cap) {
    int this_cols = 0;
    char *tok = strtok(line, delims);
    while (tok) {
        char *endptr = NULL;
        double v = strtod(tok, &endptr);
        if (endptr == tok || (v != v) || (v > DBL_MAX) || (v < -DBL_MAX))  {
            free(*vals);
            handle_error();
        }
        *vals = ensure_capacity(*vals, cap, *len + 1);
        (*vals)[(*len)++] = v;
        this_cols++;
...
double *read_vals_rows_cols(const char *path, int *rows, int *cols, size_t *vals_len_out) {
    ...
    *rows = 0; *cols = -1;
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL) {
        ...
        this_cols = parse_line(buffer, delims, &vals, &vals_len, &vals_cap);
    ...
    if ((size_t)(*rows) * (size_t)(*cols) != vals_len) {
        free(vals);
        handle_error();
    }

    *vals_len_out = vals_len;
    return vals;
}
